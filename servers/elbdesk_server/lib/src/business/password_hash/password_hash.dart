...
enum _PasswordHashType {
  ...
  factory _PasswordHashType.fromPasswordHash(String passwordHash) {
    final passwordHashParts = passwordHash.split(r'$');
...
class PasswordHash {
  ...
  ///
  /// If the password does not match the hash, the [onValidationFailure]
  /// function
  ...
  static Future<String> legacyHash(
    ...
    String? email,
      ...
      Isolate.run(
        () => _LegacyPasswordHashGenerator(salt: salt, email: email)
  ...
  /// number generation. If set to false (default value), an error will be
  /// thrown
