...
class Sqids {
  ...
  Sqids({
    ...
    this.blocklist = _filterBlocklist(blocklist ?? _blocked, alphabet);
  ...
  List<int> decode(String id) {
    ...
    final alphabetChars = this.alphabet.split('');
    ...
    final prefix = id[0];
    final offset = this.alphabet.indexOf(prefix);
    var alphabet =
    ...
    while (slicedId.isNotEmpty) {
      final separator = alphabet.substring(0, 1);

      final chunks = slicedId.split(separator);
  ...
  String encodeNumbers(List<int> numbers, [int increment = 0]) {
    ...
    var alphabet =
        this.alphabet.substring(offset) + this.alphabet.substring(0, offset);
    final prefix = alphabet[0];
    alphabet = alphabet.split('').reversed.join('');
    final ret = <String>[prefix];
    ...
    if (minLength > id.length) {
      ...
      for (var i = minLength - id.length; i > 0; i -= alphabet.length) {
        alphabet = shuffle(alphabet);
        id +=
  ...
  static Set<String> _filterBlocklist(Set<String> blocklist, String alphabet) {
    ...
    return blocklist
        ...
        .where(
          (word) => Set.from(word.split('')).difference(alphabetChars).isEmpty,
        )
        .toSet();
  }
}
...
const _alphabet =
    'Uw7pGNHq9kQJRI6YVtMxLjfvs8X1o0KhzCr3A5ybadZD2OBgPFcEe4TiSnWlmu';
