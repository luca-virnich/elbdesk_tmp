...
import 'package:elbdesk_ui/src/features/color_picker/colors.dart';
...
const String kValidHexPattern = '^#?[0-9a-fA-F]{1,8}';
...
Color colorFromRgb(int red, int green, int blue, int alpha) {
  final correctAlpha = (alpha / 100 * 255).round();
  return Color.fromARGB(correctAlpha, red, green, blue);
}

(String, String, String, String) colorToRgb(Color color) {
  return (
    color.red.toString(),
    color.green.toString(),
    color.blue.toString(),
    (color.alpha / 255 * 100).round().toString()
  );
}

(String, String, String, String) colorToHsl(Color color) {
  final hslColor = HSLColor.fromColor(color);
  final hueDegrees = hslColor.hue.toString();
  final saturationToPercentString =
      (hslColor.saturation * 100).round().toString();
  final lightnessToPercentString =
      (hslColor.lightness * 100).round().toString();
  return (
    hueDegrees,
    saturationToPercentString,
    lightnessToPercentString,
    color.alpha.toString()
  );
}

Color colorFromHsl(int hue, int saturation, int lightness, int alpha) {
  final s = saturation / 100;
  final l = lightness / 100;
  final a = alpha / 100;
  return HSLColor.fromAHSL(a, hue.toDouble(), s, l).toColor();
}
