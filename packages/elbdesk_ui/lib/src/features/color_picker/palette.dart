...
// TODO(all): Probably needs to be refactored soon because of deprecated colors
...
class HSVWithValueColorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      rect,
      Paint()..color = Colors.black.withAppOpacity(1 - hsvColor.value),
    );
...
class TrackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    switch (trackType) {
      ...
      case TrackType.red:
        final List<Color> colors = [
          hsvColor.toColor().withRed(0).withAppOpacity(1.0),
          hsvColor.toColor().withRed(255).withAppOpacity(1.0),
        ];
        final Gradient gradient = LinearGradient(colors: colors);
        canvas.drawRect(rect, Paint()..shader = gradient.createShader(rect));
      case TrackType.green:
        final List<Color> colors = [
          hsvColor.toColor().withGreen(0).withAppOpacity(1.0),
          hsvColor.toColor().withGreen(255).withAppOpacity(1.0),
        ];
        final Gradient gradient = LinearGradient(colors: colors);
        canvas.drawRect(rect, Paint()..shader = gradient.createShader(rect));
      case TrackType.blue:
        final List<Color> colors = [
          hsvColor.toColor().withBlue(0).withAppOpacity(1.0),
          hsvColor.toColor().withBlue(255).withAppOpacity(1.0),
        ];
        final Gradient gradient = LinearGradient(colors: colors);
        canvas.drawRect(rect, Paint()..shader = gradient.createShader(rect));
      case TrackType.alpha:
        final List<Color> colors = [
          hsvColor.toColor().withAppOpacity(0.0),
          hsvColor.toColor().withAppOpacity(1.0),
        ];
        final Gradient gradient = LinearGradient(colors: colors);
        canvas.drawRect(rect, Paint()..shader = gradient.createShader(rect));
    }
...
class ThumbPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (thumbColor != null) {
      canvas.drawCircle(
        Offset(0.0, size.height * 0.7),
        size.height * (fullThumbColor ? 0.5 : 0.5),
        Paint()
          ..color = thumbColor!
          ..style = PaintingStyle.fill,
      );
...
class _ColorPickerLabelState extends State<ColorPickerLabel> {
  ...
  List<Widget> colorValueLabels() {
    ...
    return [
      for (final String item in _colorTypes[_colorType] ?? [])
      ...
      builder: (BuildContext context, BoxConstraints box) {
        ...
        switch (trackType) {
          ...
          case TrackType.value:
            ...
            thumbColor =
                HSVColor.fromAHSV(1.0, hsvColor.hue, 1.0, hsvColor.value)
                    .toColor();
          case TrackType.lightness:
          ...
          case TrackType.red:
            ...
            thumbColor = hsvColor.toColor().withAppOpacity(1.0);
          case TrackType.green:
            thumbOffset +=
          ...
          case TrackType.alpha:
            ...
            thumbColor = hsvColor.toColor().withAppOpacity(hsvColor.alpha);
        }
...
class ColorPickerArea extends StatelessWidget {
  ...
  void _handleGesture(
    ...
    final double horizontal = localOffset.dx.clamp(0, width);
    final double vertical = localOffset.dy.clamp(0, height);
