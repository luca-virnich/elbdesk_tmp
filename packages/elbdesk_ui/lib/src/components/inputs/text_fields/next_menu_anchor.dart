...
// ignore_for_file: cascade_invocations, lines_longer_than_80_chars, prefer_asserts_with_message
...
/// The type of builder function used by [NextMenuAnchor.builder] to build the
/// widget that the [NextMenuAnchor] surrounds.
///
/// The `context` is the context that the widget is being built in.
///
/// The `controller` is the [NextMenuController] that can be used to open and close
/// the menu with.
///
/// The `child` is an optional child supplied as the [NextMenuAnchor.child]
...
///
/// When creating a menu with [MenuBar] or a [SubmenuButton], a [NextMenuAnchor] is
/// not needed, since they provide their own internally.
///
/// The [NextMenuAnchor] is meant to be a slightly lower level interface than
/// [MenuBar], used in situations where a [MenuBar] isn't appropriate, or to
/// construct widgets or screen regions that have submenus.
///
/// {@tool dartpad}
/// This example shows how to use a [NextMenuAnchor] to wrap a button and open a
...
///
/// {@tool dartpad}
/// This example shows how to use a [NextMenuAnchor] to create a cascading context
...
/// that clicks on the [NextMenuAnchor] area will cause the menus to be closed.
///
/// ** See code in examples/api/lib/material/menu_anchor/menu_anchor.1.dart **
/// {@end-tool}
///
/// {@tool dartpad}
/// This example demonstrates a simplified cascading menu using the [NextMenuAnchor]
/// widget.
///
/// ** See code in examples/api/lib/material/menu_anchor/menu_anchor.3.dart **
/// {@end-tool}
class NextMenuAnchor extends StatefulWidget {
  /// Creates a const [NextMenuAnchor].
  ///
  ...
  /// other widgets.
  final NextMenuController? controller;
  ...
  /// [NextMenuAnchor] surrounds.
  ///
  /// When provided, the menu will follow the widget that this [NextMenuAnchor]
  ...
  /// doesn't cause it to close unless [NextMenuController.close] is called.
  ///
  /// For menus that are positioned using [NextMenuController.open]'s `position`
  ...
  /// Determine if the menu panel can be wrapped by a [UnconstrainedBox] which
  /// allows the panel to render at its "natural" size.
  ///
  /// Defaults to true as it allows developers to render the menu panel at the
  /// size it should be. When it is set to false, it can be useful when the menu
  /// should be constrained in both main axis and cross axis, such as a
  /// [DropdownMenu].
  final bool crossAxisUnconstrained;

  /// A list of children containing the menu items that are the contents of the
  /// menu surrounded by this [NextMenuAnchor].
  ///
  /// {@macro flutter.material.MenuBar.shortcuts_note}
  final List<Widget> menuChildren;

  /// The widget that this [NextMenuAnchor] surrounds.
  ///
  /// Typically this is a button used to open the menu by calling
  /// [NextMenuController.open] on the `controller` passed to the builder.
  ///
  /// If not supplied, then the [NextMenuAnchor] will be the size that its parent
  /// allocates for it.
  ...
  @override
  State<NextMenuAnchor> createState() => _NextMenuAnchorState();
...
class _NextMenuAnchorState extends State<NextMenuAnchor> {
  ...
  // view's edges.
  final GlobalKey<_NextMenuAnchorState> _anchorKey =
      GlobalKey<_NextMenuAnchorState>(
    debugLabel: kReleaseMode ? null : 'MenuAnchor',
  );
  _NextMenuAnchorState? _parent;
  late final FocusScopeNode _menuScopeNode;
  NextMenuController? _internalMenuController;
  final List<_NextMenuAnchorState> _anchorChildren = <_NextMenuAnchorState>[];
  ...
  NextMenuController get _menuController =>
  ...
  void initState() {
    ...
    if (widget.controller == null) {
      _internalMenuController = NextMenuController();
    }
  ...
  void didChangeDependencies() {
    ...
    final newParent = _NextMenuAnchorState._maybeOf(context);
  ...
  @override
  void didUpdateWidget(NextMenuAnchor oldWidget) {
    ...
    if (oldWidget.controller != widget.controller) {
      ...
      if (widget.controller != null) {
        ...
        _internalMenuController = NextMenuController().._attach(this);
      }
    }
  ...
  Widget build(BuildContext context) {
    var contents = _buildContents(context);
  ...
  FocusNode? get _firstItemFocusNode {
    ...
    final policy = FocusTraversalGroup.maybeOf(_menuScopeNode.context!) ??
  ...
  void _addChild(_NextMenuAnchorState child) {
  ...
  void _removeChild(_NextMenuAnchorState child) {
  ...
  _NextMenuAnchorState get _root {
    var anchor = this;
  ...
  /// Open the menu, optionally at a position relative to the [NextMenuAnchor].
  ///
  ...
  /// the local coordinates of the [NextMenuAnchor], ignoring any
  /// [MenuStyle.alignment] and/or [NextMenuAnchor.alignmentOffset] that were
  /// specified.
  ...
  void _closeChildren({bool inDispose = false}) {
    ...
    for (final child in List<_NextMenuAnchorState>.from(_anchorChildren)) {
  ...
  // changes.
  static _NextMenuAnchorState? _maybeOf(BuildContext context) {
    return context
...
/// A controller to manage a menu created by a [MenuBar] or [NextMenuAnchor].
///
/// A [NextMenuController] is used to control and interrogate a menu after it has
/// been created, with methods such as [open] and [close], and state accessors
/// like [isOpen].
///
/// See also:
///
/// * [NextMenuAnchor], a widget that defines a region that has submenu.
...
class NextMenuController {
  /// The anchor that this controller controls.
  ///
  /// This is set automatically when a [NextMenuController] is given to the anchor
  /// it controls.
  _NextMenuAnchorState? _anchor;
  ...
  ///
  /// Associating with a menu is done by passing a [NextMenuController] to a
  /// [NextMenuAnchor]. A [NextMenuController] is also be received by the
  /// [NextMenuAnchor.builder] when invoked.
  ///
  /// If the menu's anchor point (either a [MenuBar] or a [NextMenuAnchor]) is
  ...
  /// the coordinate space of the [NextMenuAnchor] this controller is attached to.
  ///
  /// If given, the `position` will override the [NextMenuAnchor.alignmentOffset]
  /// given to the [NextMenuAnchor].
  ///
  /// If the menu's anchor point (either a [MenuBar] or a [NextMenuAnchor]) is
  /// scrolled by an ancestor, or the view changes size, then any open menu will
  /// automatically close.
  ...
  void _attach(_NextMenuAnchorState anchor) {
    _anchor = anchor;
  }

  void _detach(_NextMenuAnchorState anchor) {
    if (_anchor == anchor) {
      _anchor = null;
    }
  }
...
/// Menus using [NextMenuItemButton] can have a [SingleActivator] or
/// [CharacterActivator] assigned to them as their [NextMenuItemButton.shortcut],
...
/// See also:
///
/// * [NextMenuAnchor], a widget that creates a region with a submenu and shows it
///   when requested.
/// * [SubmenuButton], a menu item which manages a submenu.
/// * [NextMenuItemButton], a leaf menu item which displays the label, an optional
...
class MenuBar extends StatelessWidget {
  ...
  /// The [NextMenuController] to use for this menu bar.
  final NextMenuController? controller;
...
/// A button for use in a [MenuBar], in a menu created with [NextMenuAnchor], or on
...
/// * [CallbackShortcuts] to define shortcuts that call a callback without
///   involving [Actions].
class NextMenuItemButton extends StatefulWidget {
  /// Creates a const [NextMenuItemButton].
  ///
  /// The [child] attribute is required.
  const NextMenuItemButton({
    super.key,
    this.onPressed,
    this.onHover,
  ...
  /// The optional shortcut that selects this [NextMenuItemButton].
  ///
  /// {@macro flutter.material.MenuBar.shortcuts_note}
  final MenuSerializableShortcut? shortcut;

  /// An optional Semantics label, applied to the entire [NextMenuItemButton].
  ///
  /// A screen reader will default to reading the derived text on the
  /// [NextMenuItemButton] itself, which is not guaranteed to be readable.
  ...
  /// Determines if the menu will be closed when a [NextMenuItemButton]
  /// is pressed.
  ///
  /// Defaults to true.
  /// {@endtemplate}
  ...
  ///
  /// If the menu item button is a descendent of [NextMenuAnchor] or [MenuBar], then
  ...
  @override
  State<NextMenuItemButton> createState() => _NextMenuItemButtonState();
  ...
  /// A static convenience method that constructs a [NextMenuItemButton]'s
  ...
  /// [NextMenuItemButton], as well as its overlay color, with all of the standard
...
class _NextMenuItemButtonState extends State<NextMenuItemButton> {
  // If a focus node isn't given to the widget, then we have to manage our own.
  FocusNode? _internalFocusNode;
  FocusNode get _focusNode => widget.focusNode ?? _internalFocusNode!;
  _NextMenuAnchorState? get _anchor => _NextMenuAnchorState._maybeOf(context);
  ...
  @override
  void didUpdateWidget(NextMenuItemButton oldWidget) {
  ...
  @override
  Widget build(BuildContext context) {
    final background = context.appTheme.buttonStyles.secondary.backgroundColor;
  ...
  void _handleFocusChange() {
    if (!_focusNode.hasPrimaryFocus) {
      ...
      _NextMenuAnchorState._maybeOf(context)?._closeChildren();
    }
  }
  ...
  void _handleSelect() {
    ...
    if (widget.closeOnActivate) {
      _NextMenuAnchorState._maybeOf(context)?._root._close();
    }
  ...
  void _createInternalFocusNodeIfNeeded() {
    if (widget.focusNode == null) {
      ...
      assert(() {
        _internalFocusNode?.debugLabel = '$NextMenuItemButton(${widget.child})';
        return true;
      }());
    }
  }
...
/// A menu item that combines a [Checkbox] widget with a [NextMenuItemButton].
///
...
/// - [NextMenuAnchor], a widget that defines a region which can host a cascading
///   menu.
class CheckboxMenuButton extends StatelessWidget {
  ...
  ///
  /// Non-null properties of this style override the corresponding properties in
  /// [NextMenuItemButton.themeStyleOf] and [NextMenuItemButton.defaultStyleOf].
...
/// A menu item that combines a [Radio] widget with a [NextMenuItemButton].
///
...
/// [NextMenuItemButton], it shows a label with an optional leading or trailing
...
/// See also:
///
/// * [NextMenuItemButton], a widget that represents a leaf menu item that does not
///   host a submenu.
...
class SubmenuButton extends StatefulWidget {
  ...
  /// An optional [NextMenuController] for this submenu.
  final NextMenuController? controller;
  ...
  /// top of the [NextMenuAnchor] region.
  final Offset? alignmentOffset;
  ...
  ///
  /// These can be any widget, but are typically either [NextMenuItemButton] or
...
class _SubmenuButtonState extends State<SubmenuButton> {
  ...
  NextMenuController get _menuController =>
      widget.controller ?? _internalMenuController!;
  _NextMenuAnchorState? get _parent => _NextMenuAnchorState._maybeOf(context);
  FocusNode? _internalFocusNode;
  ...
  void didUpdateWidget(SubmenuButton oldWidget) {
    ...
    if (widget.controller != oldWidget.controller) {
      _internalMenuController =
          (oldWidget.controller == null) ? null : NextMenuController();
    }
  }

  @override
  Widget build(BuildContext context) {
    var menuPaddingOffset = widget.alignmentOffset ?? Offset.zero;
    final menuPadding = _computeMenuPadding(context);
    final orientation = _parent?._orientation ?? Axis.vertical;
  ...
  EdgeInsets _computeMenuPadding(BuildContext context) {
    final insets = widget.menuStyle?.padding ??
...
class _SubmenuDirectionalFocusAction extends DirectionalFocusAction {
  ...
  _NextMenuAnchorState get _anchor => submenu._menuController._anchor!;
  FocusNode get _buttonFocusNode => submenu._buttonFocusNode;
  _NextMenuAnchorState? get _parent => _anchor._parent;
  ...
  void invoke(DirectionalFocusIntent intent) {
    ...
    final directionality = Directionality.of(submenu.context);
    switch ((_orientation, directionality, intent.direction)) {
      ...
      case (Axis.horizontal, _, TraversalDirection.down):
        if (isSubmenu) {
          ...
          final firstItem = _anchor._firstItemFocusNode;
      ...
      case (Axis.horizontal, _, TraversalDirection.up):
        if (isSubmenu) {
          ...
          final lastItem = _anchor._lastItemFocusNode;
...
/// See also:
///
///  * [NextMenuAnchor], a widget that hosts a cascading submenu.
...
class DismissMenuAction extends DismissAction {
  ...
  DismissMenuAction({required this.controller, required this.onDismiss});

  /// The [NextMenuController] associated with the menus that should be closed.
  final NextMenuController controller;
  final VoidCallback onDismiss;
...
/// [ShortcutActivator]).
///
/// This helper class is typically used by the [NextMenuItemButton] and
...
class _MenuAnchorScope extends InheritedWidget {
  ...
  final _NextMenuAnchorState anchor;
  final bool isOpen;
...
/// MenuBar-specific private specialization of [NextMenuAnchor] so that it can act
/// differently in regards to orientation, how open works, and what gets built.
class _MenuBarAnchor extends NextMenuAnchor {
  ...
  @override
  State<NextMenuAnchor> createState() => _MenuBarAnchorState();
}

class _MenuBarAnchorState extends _NextMenuAnchorState {
  late final Map<Type, Action<Intent>> actions = <Type, Action<Intent>>{
    DismissIntent: DismissMenuAction(
      controller: _menuController,
      onDismiss: widget.onDismiss,
  ...
  bool get _isOpen {
    ...
    for (final child in _anchorChildren) {
      if (child._isOpen) {
        return true;
      }
    }
...
/// [NextMenuAnchor] or [MenuBar]. Provided menu items such as [NextMenuItemButton] and
...
/// [NextMenuItemButton] or [SubmenuButton]) and renders its child with information
...
/// accelerator markers.
///
/// The built-in menu items [NextMenuItemButton] and [SubmenuButton] already provide
...
class MenuAcceleratorLabel extends StatefulWidget {
  ...
  static String stripAcceleratorMarkers(
    ...
    // them incorrectly.
    final labelChars = label.characters;
    final ampersand = '&'.characters;
    ...
    for (var i = 0; i < labelChars.length; i += 1) {
      ...
      final character = labelChars.characterAt(i);
      if (lastWasAmpersand) {
      ...
      final acceleratorCharacter = labelChars.characterAt(i + 1);
...
class _MenuAcceleratorLabelState extends State<MenuAcceleratorLabel> {
  ...
  void didChangeDependencies() {
    ...
    _anchor = _NextMenuAnchorState._maybeOf(context);
  ...
  @override
  Widget build(BuildContext context) {
    final index = _showAccelerators ? _acceleratorIndex : -1;
    return widget.builder(context, _displayLabel, index);
  }
}

/// A label widget that is used as the label for a [NextMenuItemButton] or
/// [SubmenuButton].
///
/// It not only shows the [SubmenuButton.child] or [NextMenuItemButton.child], but if
/// there is a shortcut associated with the [NextMenuItemButton], it will display a
...
class _MenuLayout extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    if (menuPosition == null) {
      ...
      final adjustedPosition = menuPosition! + anchorRect.topLeft;
    ...
    final allowedRect = _closestScreen(subScreens, anchorRect.center);
    ...
    if (childSize.width >= allowedRect.width) {
      ...
      if (offLeftSide(x)) {
        ...
        if (parentOrientation != orientation) {
          ...
          final newX = anchorRect.right + alignmentOffset.dx;
        ...
        if (parentOrientation != orientation) {
          ...
          final newX = anchorRect.left - childSize.width - alignmentOffset.dx;
          if (!offLeftSide(newX)) {
            x = newX;
          } else {
            x = allowedRect.right - childSize.width;
    ...
    if (childSize.height >= allowedRect.height) {
      ...
      if (offTop(y)) {
        ...
        final newY = anchorRect.top - childSize.height;
        if (!offTop(newY)) {
...
class _MenuPanelState extends State<_MenuPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final widgetStyle = widget.menuStyle;
    ...
    final fixedSize = resolve<Size?>((MenuStyle? style) => style?.fixedSize);
    final maximumSize =
        resolve<Size?>((MenuStyle? style) => style?.maximumSize);
    final side = resolve<BorderSide?>((MenuStyle? style) => style?.side);
    final shape = resolve<OutlinedBorder?>((MenuStyle? style) => style?.shape)!
        .copyWith(side: side);
    final visualDensity =
    ...
    if (fixedSize != null) {
      final size = effectiveConstraints.constrain(fixedSize);
    ...
    var menuPanel = _intrinsicCrossSize(
      child: Material(
        elevation: elevation,
        shape: shape,
        color: backgroundColor,
...
class _Submenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final alignment = effectiveValue((MenuStyle? style) => style?.alignment)!;
    final padding =
        resolve<EdgeInsetsGeometry?>((MenuStyle? style) => style?.padding) ??
            EdgeInsets.zero;
    final densityAdjustment = visualDensity.baseSizeAdjustment;
...
class _MenuBarDefaultsM3 extends MenuStyle {
  ...
  WidgetStateProperty<Color?> get backgroundColor {
    return WidgetStatePropertyAll<Color?>(_colors.surfaceContainer);
  }
...
class _MenuButtonDefaultsM3 extends ButtonStyle {
  ...
  WidgetStateProperty<Color?>? get foregroundColor {
    return WidgetStateProperty.resolveWith((Set<WidgetState> states) {
      if (states.contains(WidgetState.disabled)) {
        return _colors.onSurface.withAppOpacity(0.38);
      }
  ...
  WidgetStateProperty<double>? get iconSize {
    return const WidgetStatePropertyAll<double>(24);
  }
  ...
  WidgetStateProperty<Size>? get minimumSize {
    return ButtonStyleButton.allOrNull<Size>(const Size(64, 48));
  }
  ...
  WidgetStateProperty<Color?>? get overlayColor {
    return WidgetStateProperty.resolveWith(
      (Set<WidgetState> states) {
        if (states.contains(WidgetState.pressed)) {
          return _colors.onSurface.withAppOpacity(0.1);
        }
        if (states.contains(WidgetState.hovered)) {
          return _colors.onSurface.withAppOpacity(0.08);
        }
        if (states.contains(WidgetState.focused)) {
          return _colors.onSurface.withAppOpacity(0.1);
        }
  ...
  EdgeInsetsGeometry _scaledPadding(BuildContext context) {
    var visualDensity = Theme.of(context).visualDensity;
    ...
    // padding.
    final fontSize = Theme.of(context).textTheme.labelLarge?.fontSize ?? 14.0;
    final fontSizeRatio =
