...
class _AddSingleNutzenlayoutSelectMetricsPage extends HookWidget {
  const _AddSingleNutzenlayoutSelectMetricsPage({
    ...
    return Column(
      children: [
        ...
        NextCardFormField.int(
          ...
          validator: (v) {
            if (v == null) {
              return 'Bitte geben Sie eine gültige Anzahl '
                      'an Nutzen pro Spalte ein'
                  .hc;
            }
            final asInt = int.tryParse(v);
            if (asInt == null) {
              return 'Bitte geben Sie eine gültige Anzahl an Nutzen '
                      'pro Spalte ein'
                  .hc;
            }
            if (asInt <= 0) {
...
// ! Don't delete this, will use this later in the Drucklayout
class _Kuerzung extends HookConsumerWidget {
  const _Kuerzung({
    required this.sessionId,
  });
  final String sessionId;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final type = ref.watch(
      nutzenLayoutMetricsStateProvider(sessionId).select((value) => value.type),
    );
    final drucklaenge = ref.watch(
      nutzenLayoutMetricsStateProvider(sessionId)
          .select((value) => value.drucklaenge),
    );

    final kuerzung = ref.watch(
      nutzenLayoutMetricsStateProvider(sessionId)
          .select((value) => value.kuerzung),
    );

    final mmController = useTextEditingController(
      text: kuerzung.toString(),
    );
    final prozentController = useTextEditingController(
      text: (kuerzung / drucklaenge * 100).toStringAsFixed(0),
    );

    // final customErrorMessageNotifier = useValueNotifier<String?>(null);

    useListenable(mmController);
    useListenable(prozentController);

    useEffect(
      () {
        if (type == NutzenlayoutType.rotativ) {
          final localKuerzung = Parser.parseDouble(mmController.text);
          if (localKuerzung == null) {
          ...
          final prozent = 100 - (localKuerzung / drucklaenge * 100);
          // check if prozent is a number

          prozentController.text = prozent.isNaN || prozent.isInfinite
              ? ''
              : prozent.toStringAsFixed(3);
          ...
          WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
            ref
                ...
                .updateKuerzung(localKuerzung);
          });

          // mmController.text = prozent.toStringAsFixed(4);
          return;
        }

        if (type == NutzenlayoutType.translativ) {
          final localProzent = Parser.parseDouble(prozentController.text);
          if (localProzent == null) {
            WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
              prozentController.text = '';
              mmController.text = '';
              ref
          ...
          final inMm = localProzent / 100 * drucklaenge;
          mmController.text =
              inMm.isNaN ? '' : (drucklaenge - inMm).toStringAsFixed(3);
          ...
          WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
            ref
                ...
                .updateKuerzung(drucklaenge - inMm);
          });
        }

        return null;
      },
      [mmController.text, prozentController.text, drucklaenge],
    );
    final appTheme = context.appTheme;
      ...
      children: [
        SizedBox(
          ...
          child: NextCardMultiField(
            ...
            labelPosition: NextCardFormFieldLabelPosition.left,
            fields: [
              AppSlidingControl<NutzenlayoutType>(
                backgroundColor:
                    appTheme.buttonStyles.secondary.backgroundColor,
                thumbColor: appTheme.generalColors.primary,
                children: const {
                  NutzenlayoutType.rotativ: MouseRegion(
                    cursor: SystemMouseCursors.click,
                    child: Padding(
                      padding: EdgeInsets.symmetric(horizontal: 4, vertical: 5),
                      child: AppText('Rotativ', autoSize: false),
                    ),
                  ),
                  NutzenlayoutType.translativ: MouseRegion(
                    cursor: SystemMouseCursors.click,
                    child: Padding(
                      padding: EdgeInsets.symmetric(horizontal: 4, vertical: 5),
                      child: AppText('Translativ', autoSize: false),
                    ),
                  ),
                },
                groupValue: type,
              ...
              NextCardFormField.double(
                ...
                labelPosition: NextCardFormFieldLabelPosition.left,
                readOnly: type == NutzenlayoutType.translativ,
                validationGroup: null,
                onChanged: (v) {},
                validator: (v) {
                  final drucklaenge = ref
                  ...
                  final kuerzung = ref
                      .read(nutzenLayoutMetricsStateProvider(sessionId))
                      .kuerzung;
                  if (kuerzung >= drucklaenge) {
                    return 'Die Kürzung muss kleiner als die Drucklänge sein'
                        .hc;
                  }
              ...
              NextCardFormField.double(
                ...
                onChanged: (v) {},
                labelPosition: NextCardFormFieldLabelPosition.left,
                readOnly: type == NutzenlayoutType.rotativ,
                suffix: AppText(
                  '%'.hc,
                  color: suffixColor,
                ),
        ...
        AppText(
          '  =   ${(drucklaenge - kuerzung).toStringAsFixed(3)} mm',
        ),
      ],
    );
  }
